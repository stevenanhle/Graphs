...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 ...
		 List<Integer> twoHub = new ArrayList<Integer>();
		 List<Integer> oneHub = new ArrayList<Integer>();
		 oneHub=adjListsMap.get(v);
		 for(int i: oneHub)
		 {
			List<Integer> next = new ArrayList<Integer>();
			next = adjListsMap.get(i);
			for( int j: next)
			{
			twoHub.add(j);
			}
		 }
		 System.out.println(twoHub);
		 return twoHub;
	}
